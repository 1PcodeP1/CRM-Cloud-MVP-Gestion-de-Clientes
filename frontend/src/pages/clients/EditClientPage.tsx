...
export const EditClientPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      ...
      try {
        ...
        setServerError('No se pudo encontrar el cliente o el cliente no existe');
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (data: CreateClientFormData) => {
    try {
      ...
      setSuccessMsg('La información del cliente ha sido actualizada');
      reset(data); // Resetea dirty flag
      setTimeout(() => {
        navigate(`/clients/${id}`);
      }, 1500);
    } catch (error: any) {
      setServerError('No fue posible guardar los cambios. Por favor intenta de nuevo');
    }
  };

  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const onlyNums = e.target.value.replace(/[^0-9]/g, '');
    setValue('phone', onlyNums, { shouldValidate: true, shouldDirty: true });
  };

  const handleCancel = () => {
    if (isDirty) {
      if (window.confirm('¿Deseas salir sin guardar los cambios realizados?')) {
        navigate(`/clients/${id}`);
      }
    } else {
      navigate(`/clients/${id}`);
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Modifica la información del cliente. Los cambios se reflejarán de inmediato.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={isSubmitting || (!isDirty && !successMsg)}
              className="px-5 py-2.5 rounded-xl font-medium text-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:ring-4 focus:ring-emerald-500/20 transition-all shadow-sm active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
            >
